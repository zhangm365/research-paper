...
不同应用程序进行文件读写操作时调用 `fsync()` 的实现流程，例如典型的应用场景包括数据库等。
...
具体地，当 PostgreSQL 推进新 checkpoint 操作并再次调用 fsync 时，由于之前的 fsync 已经将脏页写回错误清除，使得这次 fsync 调用返回成功，然后进一步完成 checkpoint 操作（WAL 中 REDO 指针前移），但之前的脏页数据可能并未真正写入磁盘从而导致数据的丢失。

因此，对数据文件/WAL 的 fsync 返回 EIO 时，PostgreSQL 应立即 PANIC，停止推进 `checkpoint` 并保留 WAL；不要把后续 “成功的” 重试当成所有的修改数据已安全落盘的证据。
