...
主要包括三种：`Hash Join`、`Nested Loop Join` 和 `Merge Join` 算法的原理。
...
1. 算法原理
   ...
   - 执行过程：
     ...
     2. 为构建表创建哈希表；
     3. 扫描**探测表**（probe table），在哈希表中查找匹配记录。
...
2. 执行计划分析（图 1.3d）
    ...
    **执行流程**：
    ...
    3. 根据 `S` 和 `T` 连接的结果，构建哈希表（基于结果中的 `S.b` 作为哈希键）；
...
3. 应用场景
   - **最佳场景**：适合 `S` 和 `T` 是较大的数据表且在连接键上无索引支持时。
   - 关键条件：
       - 内存充足：需要足够的内存来构建哈希表；
...
2. 执行计划分析（图 1.3b）
    ...
    **执行流程**：
    ...
    3. 使用 `Hash Join` 算法连接 `S` 和 `T`（基于 `S.c = T.d`）。具体地，构建表 `T` 的哈希表，然后在表 `S` 中查找匹配项；
...
2. 执行计划分析（图 1.3c）
    ...
    **执行流程**：
    ...
    1. 使用索引 `S.Ib` 对表 `S` 进行索引扫描（`Index Scan`）；
       ...
       - 可能原因：`S.Ib` 是**覆盖索引**，包含了列 `c` 的信息（Included Columns）。

            ```sql
            -- S.Ib可能是一个覆盖索引
            CREATE INDEX S_Ib ON S(b) INCLUDE (c);
            ```
